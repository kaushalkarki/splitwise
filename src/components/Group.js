...
const Group = () => {
  ...
  useEffect(() => {
    ...
    const fetchGroupUsersAndExpenses = async (page, settle_page) => {
      try {
        const [usersResponse, expensesResponse, settleResponse] =
          await Promise.all([
            fetch(`${API_BASE_URL}/groups/${id}/get_group_users`, {
              headers: getHeaders(token),
            }),
            fetch(
              `${API_BASE_URL}/groups/${id}/expenses?page=${page}&limit=${perPage}`,
              { headers: getHeaders(token) }
            ),
            fetch(
              `${API_BASE_URL}/groups/${id}/settle_up?page=${settle_page}&limit=${perPage}`,
              { headers: getHeaders(token) }
            ),
          ]);
        ...
        const total_page = Math.ceil(
          expensesData.meta.count / expensesData.meta.per_page
        );
  ...
  const handleEditSettle = async (expenseId) => {
    try {
      console.log(expenseId);
      const response = await fetch(`${API_BASE_URL}/settles/${expenseId}`, {
        headers: getHeaders(token),
      });
      if (response.ok) {
        const data = await response.json();
      ...
      <div className='main-content'>
        <div className='grid-item first-grid'>
          ...
          <div className='button-container'>
            ...
            <button className='green-button' onClick={openSettleModal}>
              Settle up
            </button>
          </div>
        </div>
                  ...
                  <div className='expense-item'>
                    ...
                    <div>
                      <div className='expense-payer'>
                        {expense.payer === user.id
                          ? 'You paid'
                          : userMap[expense.payer] + ' paid'}
                      </div>
                      ...
                      !expense.expense_splits.some(
                        ...
                        <div>
                          <div className='expense-payer'>
                            {expense.payer === user.id
                              ? 'You lent'
                              : userMap[expense.payer] + ' lent'}
                          </div>
                          <div className='expense-amount lent'>
                            {expense.amount}
                          </div>
                        </div>
                      ) : expense.expense_splits.some(
                          ...
                          if (
                            ...
                            return (
                              <div key={ele.user_id}>
                                ...
                                <div className='expense-amount lent'>
                                  {expense.amount - ele.user_amount}
                                </div>
                              </div>
                            );
                            ...
                            return (
                              <div key={ele.user_id}>
                                <div className='expense-payer'>
                                  {userMap[expense.payer] + ' lent you'}
                                </div>
                                <div className='expense-amount owes'>
                                  {ele.user_amount}
                                </div>
                              </div>
                            );
                    ...
                    <div className='delete-div'>
                      <button className='delete-button' onClick={handleClick}>
                        <MoreVertIcon
                          ...
                          aria-haspopup='true'
                          onClick={(event) =>
                            handleMenuClick(event, expense.id)
                          }
                        />
                      </button>
                      <Menu
              ...
              <div key={balance[0]} className='balance-item'>
                <div className='user-info'>
                  ...
                  <div
                    ...
                    style={{ color: balance[1] < 0 ? 'red' : 'green' }}
                  >
                    {balance[1] < 0
                      ? `owes INR ${Math.abs(balance[1])}`
                      : `gets back INR ${balance[1]}`}
                  </div>
                </div>
              </div>
            ))}
              ...
              summary.map((summary, index) => (
                <div className='summary-item' key={index}>
                  <div className='summary-balance'>
                    <div>
                      <b>{userMap[summary[0]]}</b> will pay{' '}
                      <b>{userMap[summary[1]]}</b> Rs.{summary[2]}
                    </div>
                  </div>
                </div>
              ))
